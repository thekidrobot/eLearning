...
if (isset($_POST["upload"]))
{
 ...
 for($i=0;$i<count($_FILES['image']['name']);$i++)
 {
  ...
  if((!empty($_FILES["image"]['name'][$i])) && ($_FILES['image']['error'][$i] == 0))
  {
   ...
   {
    $error= "Extension no permitida. Solamente se permiten imagenes, archivos de MSOffice, PDF's y Archivos de audio en MP3.";
   }
   else
   {
...
if ($_GET['IdVideo']){ ?>
 <h3>Subir Archivos (Se permiten archivos de MSOffice, PDF's y Archivos de audio en MP3).</h3>
 <fieldset>
